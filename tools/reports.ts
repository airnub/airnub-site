...
const APPS: AppConfig[] = [
  {
    key: 'airnub',
    port: 3101,
    routes: ['/', '/work', '/projects', '/hire', '/about', '/contact', '/privacy', '/robots.txt', '/sitemap.xml', '/opengraph-image'],
    start: '/',
  },
  {
    key: 'speckit',
...
function sitemapHostOk(robotsTxt: string, expectedOrigin?: string) {
  ...
  try {
    const want = new URL(expectedOrigin).origin;
    return sitemaps.every((url) => new URL(url).origin === want);
  } catch {
    return false;
  }
}
  ...
  for (const config of APPS) {
    ...
    for (const route of config.routes) {
      ...
      try {
        ...
        if (route === '/robots.txt') {
          const robotsText = String(res.html ?? '');
          const sitemapUrls = extractRobotsSitemaps(robotsText);
          const hasSitemap = sitemapUrls.length > 0;
          const expectedOriginRaw =
            config.key === 'airnub'
              ? process.env.NEXT_PUBLIC_SITE_URL ?? CANONICAL_ORIGINS.airnub
              : config.key === 'speckit'
              ? process.env.NEXT_PUBLIC_SITE_URL_SPECKIT
              : config.key === 'adf'
              ? process.env.NEXT_PUBLIC_SITE_URL_ADF
              : undefined;
          const expectedOrigin = expectedOriginRaw?.trim() ? expectedOriginRaw : undefined;
          const hostOk = expectedOrigin ? sitemapHostOk(robotsText, expectedOrigin) : null;
          routeResult.robots = { hasSitemap, hostOk, urls: sitemapUrls } satisfies RobotsCheck;
          if (!hasSitemap) {
            report.outstanding.push({
              priority: 2,
              title: `${config.key} robots missing sitemap line`,
            });
          } else if (hostOk === false) {
            report.outstanding.push({
              priority: 2,
